...
function AdminUpdate() {
  ...
  return (
    <div className={`min-h-screen flex flex-col pt-10 ${darkMode ? 'bg-[#121212] text-white' : 'bg-white text-black'} p-4 relative`}>
 
     <div className="absolute inset-0 z-0 pointer-events-none">
      <div className="absolute w-full h-full bg-gradient-to-r from-purple-900/20 via-indigo-900/10 to-blue-900/20 animate-pulse opacity-40 blur-3xl" />
    </div>
     <ParticleEffect darkMode={darkMode}></ParticleEffect>
      ...
      <div className="container mx-auto p-6">
        <h1 className="text-3xl font-bold mb-4">Delete Problems</h1>
        {fetchLoading ? (
          ...
          <div className="flex justify-center items-center">
            <div className="text-center text-lg max-w-md">No problems available to delete.</div>
          </div>
        ) : (
          <table className="table w-full max-w-4xl mx-auto">
            <thead>
              <tr>
                <th className="w-1/12 text-gray-400">#</th>
                <th className="w-4/12 text-gray-400">Title</th>
                <th className="w-2/12 text-gray-400">Difficulty</th>
                <th className="w-2/12 text-gray-400">Tags</th>
                <th className="w-3/12 text-gray-400">Actions</th>
              </tr>
            </thead>
            <tbody>
              {problems.map((problem, index) => (
                <tr
                  key={problem._id}
                  className={` ${darkMode ? 'hover:bg-gray-700 transition-all duration-200' : 'hover:bg-gray-200 transition-all duration-200'}`}
                >
                  <th>{index + 1}</th>
                  <td>
                  <td>
                  ...
                  <td>
                    <button
                      ...
                      className="btn btn-sm btn-error btn-outline hover:bg-error hover:text-white"
                    >
                      Delete
                    </button>
                  </td>
