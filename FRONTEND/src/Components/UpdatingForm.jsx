...
function UpdatingForm() {
  ...
  const { id } = useParams();
  const languageMap = ['cpp', 'java', 'javascript'];
    ...
    const fetchProblemById = async () => {
      try {
        ...
        reset({
          ...
          tags: Array.isArray(data.tags) ? data.tags[0] || 'array' : data.tags || 'array',
          ...
          startCode: [
            { language: 'c++', initialCode: data.startCode?.find(c => c.language.toLowerCase() === 'c++')?.initialCode || '' },
            { language: 'java', initialCode: data.startCode?.find(c => c.language.toLowerCase() === 'java')?.initialCode || '' },
            { language: 'javascript', initialCode: data.startCode?.find(c => c.language.toLowerCase() === 'javascript')?.initialCode || '' }
          ],
          referenceSolution: [
            { language: 'c++', completeCode: data.referenceSolution?.find(c => c.language.toLowerCase() === 'c++')?.completeCode || '' },
            { language: 'java', completeCode: data.referenceSolution?.find(c => c.language.toLowerCase() === 'java')?.completeCode || '' },
            { language: 'javascript', completeCode: data.referenceSolution?.find(c => c.language.toLowerCase() === 'javascript')?.completeCode || '' }
          ]
        });
      } catch (err) {
        setAlert({ type: 'error', message: 'Failed to fetch problem details' });
  ...
  const onSubmit = async (data) => {
    try {
      ...
      setAlert({ type: 'success', message: 'Problem updated successfully!' });
  ...
  return (
    <div className={`p-4 relative ${darkMode ? 'bg-base-100 text-white' : 'bg-white text-black'}`}>
      ...
      {alert.message && (
        <div
          role="alert"
          className={`alert ${alert.type === 'success' ? 'alert-success' : 'alert-error'} mb-4 absolute top-2.5`}
        >
          <svg
            xmlns="http://www.w3.org/2000/svg"
            ...
            <form onSubmit={handleSubmit(onSubmit)} className="space-y-6 ">
              ...
              <div className="flex gap-4">
                <button type="submit" className="btn btn-primary w-1/2" disabled={loading}>
                  {loading ? <span className="loading loading-bars loading-md"></span> : 'Update Problem'}
                </button>
                <button
                  type="button"
